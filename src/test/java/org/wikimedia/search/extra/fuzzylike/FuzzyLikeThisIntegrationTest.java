...
public class FuzzyLikeThisIntegrationTest extends AbstractPluginIntegrationTest {
    ...
    private void setup() throws IOException, InterruptedException, ExecutionException {
        ...
        indexRandom(false, doc("humans", "From time to time, it is worth wandering around the fuzzy border regions of what you do, if only to remind yourself that no human activity is an island."));
        indexRandom(false, doc("engineers", "Unfortunately, I'm an engineer. I'm always thinking about, what's the task and how do I get it done? And some of my tasks are pretty broad, and pretty fuzzy, and pretty funky, but that's the way I think."));
        indexRandom(false, doc("science", "People assume that science is a very cold sort of profession, whereas writing novels is a warm and fuzzy intuitive thing. But in fact, they are not at all different."));
        indexRandom(false, doc("nostalgia", "Nostalgia is something we think of as fuzzy. But it's pain. Pain concerning the past."));
        refresh();
    }
