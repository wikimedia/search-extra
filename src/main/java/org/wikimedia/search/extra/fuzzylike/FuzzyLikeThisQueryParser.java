...
public class FuzzyLikeThisQueryParser implements QueryParser {
    ...
    public Query parse(QueryParseContext parseContext) throws IOException, QueryParsingException {
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if ("like_text".equals(currentFieldName) || "likeText".equals(currentFieldName)) {
                    ...
                    throw new QueryParsingException(parseContext, "[flt] query does not support [" + currentFieldName + "]");
                }
            } else if (token == XContentParser.Token.START_ARRAY) {
                if ("fields".equals(currentFieldName)) {
                    fields = Lists.newLinkedList();
                    while ((token = parser.nextToken()) != XContentParser.Token.END_ARRAY) {
                        fields.add(parseContext.fieldMapper(parser.text()).names().indexName());
                    }
                } else {
                    throw new QueryParsingException(parseContext, "[flt] query does not support [" + currentFieldName + "]");
                }
        ...
        if (likeText == null) {
            throw new QueryParsingException(parseContext, "fuzzy_like_this requires 'like_text' to be specified");
        }
        ...
        if (fields == null) {
            ...
            throw new QueryParsingException(parseContext, "fuzzy_like_this requires 'fields' to be non-empty");
        }
        for (Iterator<String> it = fields.iterator(); it.hasNext(); ) {
            final String fieldName = it.next();
            if (!Analysis.generatesCharacterTokenStream(analyzer, fieldName)) {
                if (failOnUnsupportedField) {
                    throw new ElasticsearchParseException("more_like_this doesn't support binary/numeric fields: [" + fieldName + "]");
                } else {
                    it.remove();
                }
            }
